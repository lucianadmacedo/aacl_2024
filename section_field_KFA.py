...
# Step 4: Group the DataFrame by field and section
grouped_by_field_section = df.groupby(['Field', 'Section'])

# Step 5: Calculate t-tests for each section from a field against all other sections of the same type from other fields
for (field_target, section_target), data_target in grouped_by_field_section:
    # Define target and reference groups
    group1 = data_target.select_dtypes(include=[np.number])
    group2 = df[(df['Field'] != field_target) & (df['Section'] == section_target)].select_dtypes(include=[np.number])
    ...
    result_df = pd.DataFrame({'Field_Target': [field_target] * len(means_target),
                              'Section_Target': [section_target] * len(means_target),
    ...
    result_df.to_csv(f"{field_target}_{section_target}_vs_all_other_{section_target}_sections.csv", index=False)
